...
3. **Given** a git hook already exists at the target hook path from another tool, **When** the operator runs the installer, **Then** it refuses to overwrite it and instructs the operator to integrate manually. A hook this installer wrote on an earlier run is not from another tool: re-running the installer refreshes it in place and succeeds.
...
- **FR-014**: The atom's installer MUST refuse to overwrite a pre-existing git hook at either target hook path that another tool created, rather than silently replacing it. It MUST recognize a hook it wrote itself on an earlier run — by the signature line that follows the shebang, never by a symlink — and refresh it in place, so that re-running the installer is idempotent.
...
- The `graphify` CLI (package `graphifyy`) may be present already (installed via `uv tool install graphifyy`) or offered by the atom's installer via the opt-in prompt in FR-011; this atom does not silently mutate the operator's environment.
