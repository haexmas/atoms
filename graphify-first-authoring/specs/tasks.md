...
- [X] T005 [US1] Write the molecule manifest in `.specify/molecules/graphify-first-authoring/manifest.json` declaring `atoms.behavior` and the `install.py` `install_hook`, conforming to the extended molecule-manifest v4 schema
...
- [X] T013 [P] [US3] Implement snapshot logic in `.specify/molecules/graphify-first-authoring/hooks/_snapshot.py`: resolve the tracked source worktree by matching its HEAD to the new checkout HEAD, accept and validate an explicit `GRAPHIFY_PARENT_WORKTREE` override, copy a complete parent `graphify-out/` containing `graph.json` recursively if no complete graph exists locally, replace an incomplete destination directory, and no-op for an absent/incomplete parent graph or complete destination (contracts/git-hooks.md, FR-008)
...
- [X] T016 [US3] Integration test in `tests/atoms/graphify_first_authoring/test_post_checkout_hook.py`: real `git worktree add` without an environment variable from a tracked branch with an existing `graphify-out/` produces a populated copy in the new worktree (depends on T014)
...
- [X] T026 [P] Run the full [quickstart.md](quickstart.md) walkthrough end-to-end on a scratch clone — **Verified after PR #21** re-pinned both atoms to reachable origin SHAs. A fresh `git clone https://github.com/haexmas/haex-hive` walked through steps 1–6 cleanly: `graphify --version` reports 0.8.44; `install.py` (with the harness-registration prompt declined) installed both hooks with a working shebang and left `.gitignore`'s `graphify-out/` line intact; `haex constitution show` printed the "Assembled from" preface with `443c3af` + `a83ef22`; `haex constitution assemble --llm=file` emitted the pending merge JSON with both sources at the new pins (multi-source LLM-merge path exercised end-to-end); an `--allow-empty` commit fired the post-commit hook and bootstrapped `graphify-out/` from scratch with `.meta.json.indexed_at_sha` matching the new `HEAD`; `GRAPHIFY_PARENT_WORKTREE=<parent> git worktree add` fired the post-checkout hook and copied the parent snapshot to the new worktree with a matching marker.
